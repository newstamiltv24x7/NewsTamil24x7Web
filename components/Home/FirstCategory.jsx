...
function FirstCategory({viewControl, orderedMenu = []}) {
  const FirstCategoryId = orderedMenu?.at(0)?.c_category_id;
  ...
  return (
    <Box my={3}>
      <Box pr={1.3}>
        <CommonHeader
          title={orderedMenu?.at(0)?.c_category_name}
          engTitle={`More ${orderedMenu?.at(0)?.c_category_name} News`}
          url={orderedMenu?.at(0)?.c_category_slug_english_name}
        />
      </Box>
      ...
      >
        <Link href={`/news/${orderedMenu?.at(0)?.c_category_slug_english_name || '#'}`}>
          <Button
            variant="outlined"
            sx={{
